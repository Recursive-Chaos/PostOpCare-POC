...
describe('Doctor Patients Management', () => {
  ...
  describe('POST /invite', () => {
    ...
    it('should invite a new patient successfully', async () => {
      ...
      (db.query as any).mockImplementation((sql: string) => {
        if (sql.includes('role = \'doctor\'')) return Promise.resolve({ rows: [{ user_id: 'doc-1' }] });
        if (sql.includes('FROM postopcare.users WHERE email')) return Promise.resolve({ rows: [] });
        return Promise.resolve({ rows: [] });
      });

      (supabaseAdmin.auth.admin.listUsers as any).mockResolvedValue({ data: { users: [] } });
      ...
      const res = await request(app)
        .post(endpoint)
        .set('Authorization', 'Bearer token')
        .send({ email: 'new@patient.com', firstName: 'New', lastName: 'Patient' });
    ...
    it('should link an existing patient to the doctor', async () => {
      ...
      (db.query as any).mockImplementation((sql: string) => {
        ...
        if (sql.includes('FROM postopcare.users WHERE email')) {
          return Promise.resolve({ rows: [{ user_id: 'existing-p-1', role: 'patient' }] });
        }
        return Promise.resolve({ rows: [] });
      });
      ...
      const res = await request(app)
        .post(endpoint)
        .set('Authorization', 'Bearer token')
        .send({ email: 'existing@patient.com' });
